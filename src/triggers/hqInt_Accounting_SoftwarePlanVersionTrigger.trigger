trigger hqInt_Accounting_SoftwarePlanVersionTrigger on Accounting_SoftwarePlanVersion__c (after insert, after update) 
{
    ...
    for(Accounting_SoftwarePlanVersion__c softPlanVer : [SELECT id,Accounting_SoftwarePlan__c,date_created__c FROM Accounting_SoftwarePlanVersion__c WHERE Accounting_SoftwarePlan__c in :mapPlanIdToLastVersionDate.keySet() AND date_created__c  != null])
    {
    ...
    for(String planId : mapPlanIdToLastVersionDate.keySet())
    {
        ...
        Accounting_Softwareplan__c softwarePlan = new Accounting_Softwareplan__c(id=planId,Date_of_Last_Version__c=mapPlanIdToLastVersionDate.get(planId),Date_of_Last_Version_Text__c=dateOfLastVerText);
