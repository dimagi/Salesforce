trigger RedFormAddedTrigger on Record_of_Emergency_Data_Form__c (after insert) {
    ...
    for (Record_of_Emergency_Data_Form__c newRedForm : Trigger.new) {
        List<SFDC_Employee__c> employees = [SELECT Id, Employee_First_Name__c, Employee_Last_Name__c, Employee_Middle_Name__c, Business_Unit__c,
        ...
        if (employees.size() > 0) {
            ...
            List<Business_Unit__c> buList = [SELECT Id, Name FROM Business_Unit__c WHERE Name =: newRedForm.Business_Unit__c];
            if (buList.size() > 0) {
                ...
                emplToUpdate.Business_Unit__c = buList.get(0).Id;
